...
const long double bd_step = 10.0l; // kg/m^3
...
const gtd::vec3 orientation{0, 0, 1};
...
void output_data(std::ostream& os,
                 ...
                 const std::vector<std::tuple<long double, long double, std::string>>& tups,
                 const ld_pair *seps) {
    os << "time_elapsed,target_comet_radius,actual_comet_radius,body_spacing,body_cor,or_x,or_y,or_z,w_x,w_y,w_z\n" <<
    dt*iterations*num_frames << ',' << comet_rad << ',' << real_rad << ',' << spacing << ',' << cor << ',' <<
    orientation.get_x() << ',' << orientation.get_y() << ',' << orientation.get_z() << ',' <<
    omega.get_x() << ',' << omega.get_y() << ',' << omega.get_z() << '\n' <<
    "bulk_density,body_mass,mean_sep,mean_com_dist,,,,,,,\n";
    for (const auto &[bd, bmass, _] : tups)
        os << bd << ',' << bmass << ',' << seps->first << ',' << seps++->second << ",,,,,,," << '\n';
}
...
int main(int argc, char **argv) {
    ...
    std::vector<std::tuple<long double, long double, std::string>> vals;//, long double, long double>> vals;
    vals.reserve(num_sims);
    while (counter++ < num_sims) {
        vals.emplace_back(bd, gtd::comet_body_mass(pair.second, bd, pair.first),
    ...
    for (auto &[rho, b_mass, path] : vals) {
        ...
        bool, bool, long double, uint64_t, const char *>,
                   ...
                   &gtd::system<long double, long double, long double, false, false, 3, 0, 0, false>::hcp_comet<false>,
                             comet_rad, comet_pos, comet_vel, spacing, b_mass, body_rad, cor,
                             orientation, omega, true, true, 1.0l, 1l, "M1:1,D1:1,T1:1");
